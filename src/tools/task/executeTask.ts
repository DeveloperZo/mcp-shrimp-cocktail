...
export async function executeTask({
  ...
  try {
    ...
    if (!task) {
      return {
        content: [
          {
            type: "text" as const,
            text: `找不到ID為 \`${taskId}\` 的任務在${projectContext}中。請確認ID是否正確。`,
          },
        ],
      };
    }
    ...
    // 檢查任務是否可以執行（依賴任務都已完成）
    const executionCheck = await canExecuteTask(taskId, projectId);
    ...
    // 評估任務複雜度
    const complexityResult = await assessTaskComplexity(taskId, projectId);
    ...
    if (task.dependencies && task.dependencies.length > 0) {
      for (const dep of task.dependencies) {
        const depTask = await getTaskById(dep.taskId, projectId);
        if (depTask) {
