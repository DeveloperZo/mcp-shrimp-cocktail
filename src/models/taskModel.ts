...
// SSE notification function - will be set by index.ts when GUI is enabled
let sseNotifier: ((options: {
  projectId?: string;
...
// Function to set the SSE notifier from the main server
export function setSseNotifier(notifier: typeof sseNotifier) {
  sseNotifier = notifier;
}

// Helper functions to replace dependencies on projectModel and planModel

/**
 * Resolve project name to project ID by reading projects.json
 */
async function resolveProjectId(projectNameOrId: string): Promise<string> {
  try {
...
function getPlanPath(projectId: string, planId: string): string {
  return path.join(getProjectPath(projectId), "plans", planId);
}

function getPlanTasksFilePath(projectId: string, planId: string): string {
  return path.join(getPlanPath(projectId, planId), TASKS_FILE_NAME);
}

function getProjectTasksFilePath(projectId: string): string {
  return path.join(getProjectPath(projectId), TASKS_FILE_NAME);
}

// Helper function to send SSE updates if available
function notifyGuiUpdate(options: {
  projectId?: string;
  updateType?: 'tasks' | 'projects' | 'plans' | 'both';
  action?: 'created' | 'updated' | 'deleted';
  data?: any;
  ...
  if (sseNotifier) {
    try {
      ...
      // Silently ignore SSE errors to not break core functionality
    }
  }
}
...
// 讀取所有任務 (now plan-aware)
async function readTasks(projectId?: string | null, planId?: string | null): Promise<Task[]> {
  // Use the plan-aware loader
  return await loadProjectTasks(projectId, planId);
}

// 寫入所有任務 (now plan-aware)
async function writeTasks(tasks: Task[], projectId?: string | null, planId?: string | null): Promise<void> {
  // Use the plan-aware saver
  await saveProjectTasks(tasks, projectId, planId);
}

// 獲取所有任務 (now plan-aware)
export async function getAllTasks(projectId?: string | null, planId?: string | null): Promise<Task[]> {
  return await readTasks(projectId, planId);
}

// 根據ID獲取任務 (now plan-aware)
export async function getTaskById(taskId: string, projectId?: string | null, planId?: string | null): Promise<Task | null> {
  const tasks = await readTasks(projectId, planId);
  return tasks.find((task) => task.id === taskId) || null;
}

// 創建新任務 (now plan-aware)
export async function createTask(
  ...
  tasks.push(newTask);
  await writeTasks(tasks, projectId, planId);
...
export async function updateTaskStatus(
  ...
  return await updateTask(taskId, updates, projectId, planId);
}

// 更新任務摘要 (now plan-aware)
export async function updateTaskSummary(
  ...
  return await updateTask(taskId, { summary }, projectId, planId);
}

// 更新任務內容 (now plan-aware)
export async function updateTaskContent(
  ...
  // 獲取任務並檢查是否存在
  const task = await getTaskById(taskId, projectId, planId);
  ...
  // 執行更新
  const updatedTask = await updateTask(taskId, updateObj, projectId, planId);
...
export async function updateTaskRelatedFiles(
  ...
  // 執行更新
  const updatedTask = await updateTask(taskId, { relatedFiles }, projectId, planId);
...
export async function batchCreateOrUpdateTasks(
  ...
  projectId?: string | null, // Added project parameter
  planId?: string | null // Added plan parameter
): Promise<Task[]> {
  // 讀取現有的所有任務
  const existingTasks = await readTasks(projectId, planId);
  ...
  // 寫入更新後的任務列表
  await writeTasks(allTasks, projectId, planId);

  return newTasks;
}

// 檢查任務是否可以執行（所有依賴都已完成） (now plan-aware)
export async function canExecuteTask(
  ...
  const allTasks = await readTasks(projectId, planId);
...
// 清除所有任務 (now plan-aware)
export async function clearAllTasks(projectId?: string | null, planId?: string | null): Promise<{
  success: boolean;
  message: string;
  backupFile?: string;
}> {
  try {
    ...
    const timestamp = new Date()
      ...
      .replace(/:/g, "-")
      .replace(/\..+/, "");
    const backupFileName = `tasks_memory_${actualProjectId}_${actualPlanId}_${timestamp}.json`;
    ...
    // 清空任務文件
    await writeTasks([], actualProjectId, actualPlanId);
...
// New functions that support project-scoped operations while maintaining backwards compatibility

/**
 * Load tasks for a specific project and plan
 */
export async function loadProjectTasks(projectId?: string | null, planId?: string | null): Promise<Task[]> {
  try {
    ...
    // Step 4: Ensure plan directory and tasks file exist
    const planPath = getPlanPath(actualProjectId, actualPlanId);
    ...
    // Step 5: Read and parse tasks
    const data = await fs.readFile(tasksFile, "utf-8");
    const tasks = JSON.parse(data).tasks;
    
    // Step 6: Convert date strings back to Date objects and add projectId and planId
...
/**
 * Save tasks for a specific project and plan
 */
export async function saveProjectTasks(tasks: Task[], projectId?: string | null, planId?: string | null): Promise<void> {
  try {
    ...
    // Step 4: Ensure plan directory exists
    const planPath = getPlanPath(actualProjectId, actualPlanId);
    await fs.mkdir(planPath, { recursive: true });
    
    // Step 5: Add projectId and planId to all tasks if not already set
    const tasksWithIds = tasks.map(task => ({
      ...task,
      projectId: task.projectId || actualProjectId,
      planId: (task as any).planId || actualPlanId,
    }));
    
    // Step 6: Write tasks to file
    await fs.writeFile(tasksFile, JSON.stringify({ tasks: tasksWithIds }, null, 2));
    
    // Step 7: Notify GUI of task updates via SSE (if GUI is enabled)
    notifyGuiUpdate({
      projectId: actualProjectId,
      updateType: 'tasks',
      action: 'updated',
      data: { planId: actualPlanId, taskCount: tasksWithIds.length }
    });
  } catch (error) {
...
/**
 * Get a task by ID from a specific project and plan
 */
export async function getProjectTaskById(taskId: string, projectId?: string | null, planId?: string | null): Promise<Task | null> {
  const tasks = await loadProjectTasks(projectId, planId);
  return tasks.find(task => task.id === taskId) || null;
}

/**
 * Create a new task in a specific project and plan
 */
export async function createProjectTask(
  ...
  const dependencyObjects: TaskDependency[] = (taskData.dependencies || []).map((taskId) => ({
    taskId,
  }));
  ...
  tasks.push(newTask);
  await saveProjectTasks(tasks, actualProjectId, actualPlanId);
...
export async function updateProjectTask(
  ...
  // Check if task is completed and restrict updates
  if (tasks[taskIndex].status === TaskStatus.COMPLETED) {
    const allowedFields = ["summary", "relatedFiles"];
    const attemptedFields = Object.keys(updates);
    const disallowedFields = attemptedFields.filter(
  ...
  tasks[taskIndex] = {
    ...
    ...updates,
    updatedAt: new Date(),
    projectId: actualProjectId, // Ensure projectId is maintained
  };
  
  await saveProjectTasks(tasks, actualProjectId);
  return tasks[taskIndex];
}
...
export async function deleteProjectTask(
  ...
  // Check for dependent tasks within the same project
...
/**
 * Get tasks with filtering options (project-aware)
 */
export async function getFilteredTasks(options: TaskFilterOptions = {}): Promise<Task[]> {
  let tasks: Task[];
  
  if (options.projectId !== undefined) {
    // Specific project requested
    tasks = await loadProjectTasks(options.projectId);
  } else {
    // Current project
    tasks = await loadProjectTasks();
  }
  ...
  if (options.hasDependencies !== undefined) {
    if (options.hasDependencies) {
      tasks = tasks.filter(task => task.dependencies.length > 0);
    } else {
      tasks = tasks.filter(task => task.dependencies.length === 0);
    }
  }
  ...
  if (options.namePattern) {
    ...
    tasks = tasks.filter(task => 
      task.name.toLowerCase().includes(pattern) ||
      task.description.toLowerCase().includes(pattern)
    );
  }
  
  if (options.createdAfter) {
    tasks = tasks.filter(task => task.createdAt >= options.createdAfter!);
  }
  
  if (options.createdBefore) {
    tasks = tasks.filter(task => task.createdAt <= options.createdBefore!);
  }
...
// Existing functions modified to use project-aware operations under the hood
