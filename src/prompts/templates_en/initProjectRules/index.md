...
**Must generate project standards documents in an "AI Agent" folder exclusively for AI Agent operational use. The folder can contain multiple markdown files following single responsibility principle.**
...
4. **Autonomous Handling of Ambiguous Requests**: When receiving ambiguous instructions such as "update rules" without specifying concrete content, the AI **must** first attempt to independently analyze the current codebase, recent changes (if available), and existing "AI Agent" folder content to infer possible update points. List these inferred points and their rationale during the `process_thought` phase, then propose specific modification suggestions. **Strictly prohibited** from seeking clarification from users on ambiguous update requests before performing this autonomous analysis.

Based on the above guidelines, create an "AI Agent" folder and save markdown files in it. The folder can be located in:
- Project root directory
- `assets/doc/ai agent` (for Unity projects)
- `assets/docs/ai agent`
- `docs/ai agent`
- Other common documentation locations
...
**[AI Agent Action]** After completing the thought process, immediately create or update files in the "AI Agent" folder, prohibited from calling the "analyze_task" tool
