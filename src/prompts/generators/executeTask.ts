...
/**
 * Get complexity level style text
...
function getComplexityStyle(level: string): string {
  switch (level) {
    case "VERY_HIGH":
      return "⚠️ **Warning: This task has very high complexity** ⚠️";
    case "HIGH":
      return "⚠️ **Notice: This task has high complexity**";
    case "MEDIUM":
      return "**Note: This task has moderate complexity**";
    default:
      return "";
  }
}
...
export function getExecuteTaskPrompt(params: ExecuteTaskPromptParams): string {
  const { task, complexityAssessment, relatedFilesSummary, dependencyTasks, projectContext } =
    params;
  ...
  relatedFilesSummaryPrompt = generatePrompt(relatedFilesSummaryTemplate, {
    relatedFilesSummary: relatedFilesSummary || "This task has no related files.",
  });
  ...
  let prompt = generatePrompt(indexTemplate, {
    ...
    currentProject: projectContext?.currentProject || "Default Project",
    projectPath: projectContext?.projectPath || "",
    projectStatus: projectContext?.projectStatus || "Active",
  });
