...
- [🎯 Project Specification Requirements](#project-specification)
...
- [Migration Guide](MIGRATION.md): Complete guide for upgrading to explicit project specification (v1.0.20+)
...
> **⚠️ IMPORTANT BREAKING CHANGE**: Starting from version 1.0.20, this system requires **explicit project specification** for all operations. Default project behavior has been removed.

### What This Means

All task management operations now require you to explicitly specify which project you want to work with. This change provides:

- **Clear Intent**: Always know which project you're working with
...
Instead of operations that previously used default projects, you now **must** specify project names:

```bash
# ✅ CORRECT - Always specify project name
listTasks("my-project")
addTask("my-project", "Implement user authentication")
executeTask("my-project", "task-id-here")

# ❌ INCORRECT - Will now fail
listTasks()  # Error: Project name required
addTask("task description")  # Error: Project name required
```

### Project Management

Before working with tasks, ensure your projects exist:
...
createProject("My Development Project", "Main codebase project")

# Get project information
getProjectInfo("My Development Project")
```
...
1. **Check existing projects**: Use `listProjects()` to see available projects
2. **Create projects if needed**: Use `createProject()` for new work
3. **Update your workflows**: Always include project names in tool calls
4. **Refer to MIGRATION.md**: See detailed migration guidance

### Error Handling

When project names are missing or invalid, the system will:

- Provide clear error messages explaining the requirement
- Suggest available projects when applicable
- Guide you through proper project specification
- Offer examples of correct tool usage

This change ensures better organization, prevents data mix-ups, and provides a more robust multi-project development experience.
